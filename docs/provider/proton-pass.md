...
> The PAT is `pst_<token>::<key>`. The key half is a 32-byte AES key used to decrypt vault contents client-side; Proton's API only ever returns encrypted blobs. The full PAT (including the key) is submitted once per session mint to authenticate — Proton already holds this key, so no new party sees it, but the key does leave the client on the wire. Keep the PAT secret.
...
HTTP requests time out after 30 seconds, including reading the response body. A shorter caller deadline takes precedence.

When Proton returns error code 2028, session creation backs off for at least five minutes. If the `Retry-After` header specifies a longer delay in seconds, that delay applies to subsequent session attempts for the same PAT.
