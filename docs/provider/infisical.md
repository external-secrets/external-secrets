...
| `includeSecretPath` | No | `false` | When true, secret keys are prefixed with their path relative to `secretsPath`, with path separators rendered as `.` (e.g. with the default `secretsPath` of `/`, a secret `SECRET_NAME` at `/path/to/secret` becomes `path.to.secret.SECRET_NAME`) |
