...
- Development binaries are version- and checksum-pinned per platform in the Makefile and installed into `LOCALBIN` by the shared `install_tool` macro. Keep upstream update policy in `.updatecli/updatecli.d/tools.yaml`.
