...
- `make update-deps` runs the Updatecli manifests in `.updatecli.d` across all dependency ecosystems. It edits the current checkout by default; `UPDATECLI_PUBLISH=true` opts into per-ecosystem commits and pull requests through Updatecli's SCM actions. Go bumps use the generated `.updatecli.d/gomodules.yaml` (for modules) and `.updatecli.d/golang.yaml` (for toolkit) manifests. The command `hack/updatecli-gomodules`, runned periodically, generates those two updatecli manifests. The go module update through updatecli updates all deps then run a single parallel `go mod tidy` target for every module.
- Container update policy lives in `.updatecli.d/docker.yaml`: Go and kind use stable version tags only; other direct image references use digest-only pins resolved from moving tags. Keep the inventory explicit and deduplicated: Updatecli's Dockerfile autodiscovery skips digest-only references. Leave digest architecture unset to retain multi-platform image indexes.
