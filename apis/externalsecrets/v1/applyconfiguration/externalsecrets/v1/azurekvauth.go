/*
Copyright © The ESO Authors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    https://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by controller-gen. DO NOT EDIT.

package v1

import (
	metav1 "github.com/external-secrets/external-secrets/apis/meta/v1"
)

// AzureKVAuthApplyConfiguration represents a declarative configuration of the AzureKVAuth type for use
// with apply.
//
// AzureKVAuth is the configuration used to authenticate with Azure.
type AzureKVAuthApplyConfiguration struct {
	// The Azure clientId of the service principle or managed identity used for authentication.
	ClientID *metav1.SecretKeySelector `json:"clientId,omitempty"`
	// The Azure tenantId of the managed identity used for authentication.
	TenantID *metav1.SecretKeySelector `json:"tenantId,omitempty"`
	// The Azure ClientSecret of the service principle used for authentication.
	ClientSecret *metav1.SecretKeySelector `json:"clientSecret,omitempty"`
	// The Azure ClientCertificate of the service principle used for authentication.
	ClientCertificate *metav1.SecretKeySelector `json:"clientCertificate,omitempty"`
}

// AzureKVAuthApplyConfiguration constructs a declarative configuration of the AzureKVAuth type for use with
// apply.
func AzureKVAuth() *AzureKVAuthApplyConfiguration {
	return &AzureKVAuthApplyConfiguration{}
}

// WithClientID sets the ClientID field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the ClientID field is set to the value of the last call.
func (b *AzureKVAuthApplyConfiguration) WithClientID(value metav1.SecretKeySelector) *AzureKVAuthApplyConfiguration {
	b.ClientID = &value
	return b
}

// WithTenantID sets the TenantID field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the TenantID field is set to the value of the last call.
func (b *AzureKVAuthApplyConfiguration) WithTenantID(value metav1.SecretKeySelector) *AzureKVAuthApplyConfiguration {
	b.TenantID = &value
	return b
}

// WithClientSecret sets the ClientSecret field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the ClientSecret field is set to the value of the last call.
func (b *AzureKVAuthApplyConfiguration) WithClientSecret(value metav1.SecretKeySelector) *AzureKVAuthApplyConfiguration {
	b.ClientSecret = &value
	return b
}

// WithClientCertificate sets the ClientCertificate field in the declarative configuration to the given value
// and returns the receiver, so that objects can be built by chaining "With" function invocations.
// If called multiple times, the ClientCertificate field is set to the value of the last call.
func (b *AzureKVAuthApplyConfiguration) WithClientCertificate(value metav1.SecretKeySelector) *AzureKVAuthApplyConfiguration {
	b.ClientCertificate = &value
	return b
}
