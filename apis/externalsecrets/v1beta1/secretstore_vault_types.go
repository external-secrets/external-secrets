...
type VaultAWSCredentialsType string

const (
	VaultAWSCredentialsTypeCreds VaultAWSCredentialsType = "creds"
	VaultAWSCredentialsTypeSts   VaultAWSCredentialsType = "sts"
)
...
type VaultProvider struct {
	...
	// SecretEngine is the Vault secret engine used as backend. Currently supported engines
	// are key-value "kv" and Amazon Web Services "aws"
	SecretEngine VaultSecretEngine `json:"secretEngine"`
...
// VaultAWSSecretEngine is the configuration for the vault AWS Secret Engine
type VaultAWSSecretEngine struct {
	// Path is the mount path of the Vault AWS backend endpoint
	// +kubebuilder:default=aws
	Path *string `json:"path"`

	// CredentialsType is the type of credentials generated by the AWS secret engine, can
	// be either "sts" or "creds". This parameter is ignored if secretEngine is not `aws`
	// +kubebuilder:validation:Optional
	// +kubebuilder:validation:Enum="sts";"creds"
	// +kubebuilder:default:="sts"
	CredentialsType VaultAWSCredentialsType `json:"credentialsType"`
}

// VaultKeyValueSecretEngine is the configuration for the vault Key Value Secret Engine
type VaultKeyValueSecretEngine struct {
	...
	// Version is the Vault api version. This can be either "v1" or
...
// VaultSecretEngine is the configuration used to know which secret engine to use on the Vault server.
// Only one of `kv`, `aws` can be specified.
type VaultSecretEngine struct {
	// AWS configures AWS Secret Engine as backend secret provider
	// +optional
	AWS *VaultAWSSecretEngine `json:"aws,omitempty"`

	// KeyValue configures KeyValue Secret Engine as backend secret provider
	// +optional
	KeyValue *VaultKeyValueSecretEngine `json:"kv,omitempty"`
}
