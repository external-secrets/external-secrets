...
// +kubebuilder:validation:XValidation:rule="!(has(self.remoteKey) && size(self.remoteKey) > 0 && has(self.rewrite) && size(self.rewrite) > 0)",message="remoteKey and rewrite are mutually exclusive: rewrite is only supported in per-key mode (without remoteKey)"
