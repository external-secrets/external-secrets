...
// Package certificatemanager implements a provider for MWS Certificate Manager.
...
func ProviderSpec() *esv1.SecretStoreProvider {
	return &esv1.SecretStoreProvider{
		MWSCertificateManager: &esv1.MWSCertificateManagerProvider{},
	}
}
...
// Provider implements the External Secrets provider interface for MWS Certificate Manager.
...
func (p *Provider) NewClient(
	...
	authorizedKeyData, err := resolvers.SecretKeyRef(
		...
		store.GetKind(),
		namespace,
		&spec.Provider.MWSCertificateManager.Auth.AuthorizedKey,
	)
	if err != nil {
...
func (p *Provider) ValidateStore(store esv1.GenericStore) (admission.Warnings, error) {
	...
	spec := store.GetSpec()
	if spec == nil || spec.Provider == nil || spec.Provider.MWSCertificateManager == nil {
		return nil, errors.New("MWS Certificate Manager spec is not provided")
	}

	providerSpec := spec.Provider.MWSCertificateManager
