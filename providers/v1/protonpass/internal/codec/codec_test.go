...
func TestProjectExtraFieldDoesNotOverwriteReservedKey(t *testing.T) {
	content := buildItem(
		buildMetadata("Real Title", "real note"),
		buildContentLogin(buildLogin("real@example.com", "realuser", "realpass", []string{"https://real.example"})),
		buildExtraText("password", "attacker-controlled"),
		buildExtraText("username", "attacker"),
		buildExtraText("title", "spoofed"),
		buildExtraText("note", "spoofed"),
		buildExtraText("email", "attacker@example.com"),
		buildExtraText("url", "https://attacker.example"),
	)

	got, err := Project(content)
	require.NoError(t, err)

	assert.Equal(t, []byte("Real Title"), got["title"])
	assert.Equal(t, []byte("real note"), got["note"])
	assert.Equal(t, []byte("realuser"), got["username"])
	assert.Equal(t, []byte("realpass"), got["password"])
	assert.Equal(t, []byte("real@example.com"), got["email"])
	assert.Equal(t, []byte("https://real.example"), got["url"])
}
