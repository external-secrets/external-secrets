...
func TestGetSessionCanceledCallerDoesNotFailOtherWaiters(t *testing.T) {
	var calls atomic.Int32
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls.Add(1)
	...
	defer server.Close()
	pat := "pst_cancel::" + base64.RawURLEncoding.EncodeToString(make([]byte, 32))

	canceledCtx, cancelA := context.WithCancel(context.Background())
	go func() {
	...
	go func() {
		c, _ := NewClient(pat, WithBaseURL(server.URL))
		_, err := c.getSession(canceledCtx)
		canceledCh <- err
	}()
	go func() {
		c, _ := NewClient(pat, WithBaseURL(server.URL))
	...
	if err := <-canceledCh; !errors.Is(err, context.Canceled) {
		t.Fatalf("canceled caller: want context.Canceled, got %v", err)
	}
	if err := <-healthyCh; err != nil {
		t.Fatalf("healthy caller: want nil error, got %v", err)
	}
	if n := calls.Load(); n != 1 {
		t.Fatalf("server received %d calls, want 1 (singleflight collapse)", n)
	}
}
