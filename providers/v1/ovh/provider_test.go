...
func TestNewClient(t *testing.T) {
	tests := map[string]struct {
		...
		"Empty token auth": {
			errshould: "failed to create new ovh provider client: failed to configure token okms client: could not retrieve token: ovh store auth.token.tokenSecretRef cannot be empty",
			kube:      kube,
			store: &esv1.SecretStore{
				Spec: esv1.SecretStoreSpec{
					Provider: &esv1.SecretStoreProvider{
