  ...
  provider:
    crd:
      ...
      resource:
        ...
        version: v1alpha1
        kind: MyResource
      remoteNamespace: default  # SecretStore: overrides store NS for Get/List; ClusterSecretStore: scopes List only
...
### Simple mode (in-cluster, local API server)
...
GOWORK=off KUBECONFIG=~/.kube/config go test -tags e2e ./test/... -v -run TestSimpleMode
...
| `TestSecretStore_GetSecretMap` | Sub-object returned as flat map; nested objects/arrays preserved as JSON |
| `TestSecretStore_GetSecret_NestedObject` | `spec.foo` returns JSON object; `spec.foo.bar` returns scalar; `spec.nestedFields` returns JSON array; JMESPath extracts scalars from arrays |
| `TestSecretStore_GetSecretMap_FooSubobject` | `spec.foo` via GetSecretMap returns flat map with `bar`/`baz` scalars |
...
| `TestSimpleMode_NoAccessSA_NewClientFails` | `NewClient` fails when the SA has no CRD access (simple mode) |
