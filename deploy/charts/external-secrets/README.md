...
| livenessProbe.spec | object | `{"address":"","failureThreshold":5,"httpGet":{"path":"/healthz","port":"metrics"},"initialDelaySeconds":10,"periodSeconds":10,"successThreshold":1,"timeoutSeconds":5}` | The body of the liveness probe settings. |
...
| livenessProbe.spec.httpGet | object | `{"path":"/healthz","port":"metrics"}` | Handler for liveness probe. |
| livenessProbe.spec.httpGet.path | string | `"/healthz"` | Path for liveness probe. |
| livenessProbe.spec.httpGet.port | string | `"metrics"` | Set this value to 'metrics' (named port: 8080) to active liveness probes. @schema type: [string, integer] |
