...
| readinessProbe.spec | object | `{"address":"","failureThreshold":3,"httpGet":{"path":"/healthz","port":8082},"initialDelaySeconds":10,"periodSeconds":10,"successThreshold":1,"timeoutSeconds":5}` | The body of the readiness probe settings. |
...
| readinessProbe.spec.httpGet | object | `{"path":"/healthz","port":8082}` | Handler for readiness probe. |
| readinessProbe.spec.httpGet.path | string | `"/healthz"` | Path for readiness probe. |
| readinessProbe.spec.httpGet.port | int | `8082` | Port for readiness probe. Should match the livenessProbe port when both are enabled. @schema type: [string, integer] |
