...
var _ = Describe("grafana generator", Label("grafana"), func() {
	...
	BeforeEach(func() {
		...
		Eventually(func() error {
			...
			return err
		}).WithPolling(time.Second * 5).WithTimeout(time.Minute * 2).ShouldNot(HaveOccurred())
	})
	...
	cleanupServiceAccountsAfterDeletion := func(tc *testCase) {
		tc.ExternalSecret.Spec.RefreshInterval = &metav1.Duration{Duration: time.Second * 1}
		tc.AfterSync = func(secret *v1.Secret) {
			// Ensure ES has been rotated a couple of times,
			...
			Expect(err).ToNot(HaveOccurred())
			Expect(generatorStates.Items).ToNot(BeEmpty())
