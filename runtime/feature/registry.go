/*
Copyright © 2025 ESO Maintainer Team

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    https://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Package feature provides functionality related to feature flags and feature management.
package feature

import (
	"github.com/spf13/pflag"
)

// Feature contains the CLI flags that a provider exposes to a user.
// An optional Initialize func is called once the flags have been parsed.
// A provider can use this to do late-initialization using the defined cli args.
type Feature struct {
	Flags      *pflag.FlagSet
	Initialize func()
}

var features = make([]Feature, 0)

// Features returns all registered features.
func Features() []Feature {
	return features
}

// Register registers a new feature.
func Register(f Feature) {
	features = append(features, f)
}

// ProviderCapability represents a specific operation a provider can perform
type ProviderCapability string

const (
	CapabilityGetSecret              ProviderCapability = "GetSecret"
	CapabilityGetSecretMap           ProviderCapability = "GetSecretMap"
	CapabilityGetAllSecrets          ProviderCapability = "GetAllSecrets"
	CapabilityPushSecret             ProviderCapability = "PushSecret"
	CapabilityDeleteSecret           ProviderCapability = "DeleteSecret"
	CapabilitySecretExists           ProviderCapability = "SecretExists"
	CapabilityValidate               ProviderCapability = "Validate" //Client readiness? Do we need it?
	CapabilityValidateStore          ProviderCapability = "ValidateStore"
	CapabilityFindByName             ProviderCapability = "FindByName"
	CapabilityFindByTag              ProviderCapability = "FindByTag"
	CapabilityMetadataPolicyFetch    ProviderCapability = "MetadataPolicyFetch"
	CapabilityReferentAuthentication ProviderCapability = "ReferentAuthentication"
	CapabilityDeletionPolicy         ProviderCapability = "DeletionPolicy"
)

type ProviderCapabilityInfo struct {
	Capability ProviderCapability `json:"capability"`
	Notes      string             `json:"notes,omitempty"`
}

type ProviderStability string

const (
	StabilityAlpha        ProviderStability = "Alpha"
	StabilityBeta         ProviderStability = "Beta"
	StabilityStable       ProviderStability = "Stable"
	StabilityUnmaintained ProviderStability = "Unmaintained"
	StabilityDeprecated   ProviderStability = "Deprecated"
)

// ProviderMetadata represents the complete view of provider supportability state
type ProviderMetadata struct {
	Stability    ProviderStability        `json:"stability"`
	Capabilities []ProviderCapabilityInfo `json:"capabilities,omitempty"`
	Comment      string                   `json:"comment,omitempty"`
}
