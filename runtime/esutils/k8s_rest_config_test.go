...
const (
	k8sRestTestCACert = `-----BEGIN CERTIFICATE-----
...
-----END CERTIFICATE-----
`
	k8sRestTestKubeConfig = `apiVersion: v1
clusters:
- cluster:
...
`
	k8sRestTestServerURL = "https://my.test.tld"
)
	...
	tests := []struct {
		...
		},
		{
			...
			fields: fields{
				store: &esv1.KubernetesProvider{
					Server: esv1.KubernetesServer{
						CABundle: []byte(k8sRestTestCACert),
					},
				},
			},
			want:    nil,
		...
		},
		{
			...
			fields: fields{
				...
				kube: fclient.NewClientBuilder().WithObjects(&corev1.Secret{
					...
					Data: map[string][]byte{
						"cert":  []byte(k8sRestTestCACert),
						"token": []byte("mytoken"),
					},
				}).Build(),
				store: &esv1.KubernetesProvider{
					Server: esv1.KubernetesServer{
						URL: k8sRestTestServerURL,
			...
			want: &want{
				Host:        k8sRestTestServerURL,
				BearerToken: "mytoken",
				TLSClientConfig: rest.TLSClientConfig{
					CAData: []byte(k8sRestTestCACert),
				},
			},
			wantErr: false,
		},
		...
		},
		{
			...
			fields: fields{
				...
				kube: fclient.NewClientBuilder().WithObjects(&corev1.Secret{
					...
					Data: map[string][]byte{
						"config": []byte(k8sRestTestKubeConfig),
					},
				}).Build(),
				store: &esv1.KubernetesProvider{
