...
// RevertKeys reverts a secret map into a valid key map as expected by push secrets.
// Replaces encoded characters back to the original value depending on convert strategy.
func RevertKeys(strategy esv1alpha1.PushSecretConversionStrategy, in map[string][]byte) (map[string][]byte, error) {
	out := make(map[string][]byte, len(in))
	for k, v := range in {
		key := revert(strategy, k)
		if _, exists := out[key]; exists {
			return nil, fmt.Errorf("secret name collision during conversion: %s", key)
		}
		out[key] = v
...
func revert(strategy esv1alpha1.PushSecretConversionStrategy, str string) string {
	switch strategy {
