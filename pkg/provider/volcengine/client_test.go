...
func TestClient_PushSecret_should_return_readonly_error(t *testing.T) {
	client := &Client{}
	err := client.PushSecret(context.Background(), &corev1.Secret{}, &fakePushSecretData{})
	assert.Error(t, err)
	assert.Equal(t, errReadOnly, err.Error())
}

func TestClient_DeleteSecret_should_return_readonly_error(t *testing.T) {
	client := &Client{}
...
func TestClient_GetAllSecrets_should_return_not_implemented_error(t *testing.T) {
	...
	assert.Error(t, err)
	assert.Equal(t, "GetAllSecret is not implemented", err.Error())
}
