...
func (w *workloadIdentityFederation) generateExternalAccountConfig(ctx context.Context, credFile *credentialsFile) (*externalaccount.Config, error) {
	...
	if credFile != nil {
		...
		if !reflect.ValueOf(credFile.CredentialSource).IsZero() {
			...
			if credFile.CredentialSource.File != autoMountedServiceAccountTokenPath {
...
func validateCredConfig(config *externalaccount.Config, wif *esv1.GCPWorkloadIdentityFederation) error {
	var errs []error
	errs = append(errs, fmt.Errorf("invalid %s config %s", externalAccountCredentialType, wif.CredConfig.Name))
