...
var (
	awsSTSTokenURLRegex                 = regexp.MustCompile(`^http:\/\/(169\.254\.169\.254|\[fd00:ec2::254\])\/latest\/meta-data\/iam\/security-credentials$`)
	awsRegionURLRegex                   = regexp.MustCompile(`^http:\/\/(169\.254\.169\.254|\[fd00:ec2::254\])\/latest\/meta-data\/placement\/availability-zone$`)
	awsSessionTokenURLRegex             = regexp.MustCompile(`^http:\/\/(169\.254\.169\.254|\[fd00:ec2::254\])\/latest\/api\/token$`)
...
func (w *workloadIdentityFederation) readCredConfig(ctx context.Context) (*externalaccount.Config, error) {
	if w.config.CredConfig == nil {
		return nil, fmt.Errorf("workloadIdentityFederation.credConfig must not be empty")
	}
	...
	config, err := w.generateExternalAccountConfig(ctx, credFile)
	if err != nil {
		return nil, err
	}
...
func (w *workloadIdentityFederation) generateExternalAccountConfig(ctx context.Context, credFile *credentialsFile) (*externalaccount.Config, error) {
	...
	config := &externalaccount.Config{
		...
		Scopes:                   gsmapiv1.DefaultAuthScopes(),
	...
	if err := validateCredConfig(config, w.config); err != nil {
		return nil, err
	}

	if config.CredentialSource == nil && config.AwsSecurityCredentialsSupplier == nil {
		ns := w.namespace
...
// validateCredConfig is for validating the external_account credentials configurations, based on
// suggestions made at https://cloud.google.com/docs/authentication/client-libraries#external-credentials.
func validateCredConfig(config *externalaccount.Config, wif *esv1.GCPWorkloadIdentityFederation) error {
	invalidConfigErrPrefix := fmt.Sprintf("invalid %s config %s", externalAccountCredentialType, wif.CredConfig.Name)

	if config.Audience == "" {
		return fmt.Errorf("%s: audience is empty", invalidConfigErrPrefix)
	}
	...
	if config.ServiceAccountImpersonationURL != "" &&
		...
		return fmt.Errorf("%s: service_account_impersonation_url \"%s\" does not have expected value", invalidConfigErrPrefix, config.ServiceAccountImpersonationURL)
	}

	if config.TokenURL != workloadIdentityTokenURL {
		return fmt.Errorf("%s: token_url \"%s\" must match %s", invalidConfigErrPrefix, config.TokenURL, workloadIdentityTokenURL)
	}

	if config.CredentialSource != nil && config.AwsSecurityCredentialsSupplier != nil {
		return fmt.Errorf("%s: when credConfig has `credential_source` set, AwsSecurityCredentialsSupplier must be not configured", invalidConfigErrPrefix)
	}

	credSource := config.CredentialSource
	if credSource != nil {
		// restricting the use of executables from security standpoint, since executable can't be validated.
		if credSource.Executable != nil {
			return fmt.Errorf("%s: credential_source.executable.command is not allowed", invalidConfigErrPrefix)
		}
		if credSource.File == "" && credSource.URL == "" && credSource.EnvironmentID == "" {
			return fmt.Errorf("%s: one of credential_source.file, credential_source.url, credential_source.aws.url or credential_source_environment_id should be provided", invalidConfigErrPrefix)
		}
		// restricting Token source file to operator serviceaccount. But there could be valid scenarios where
		// token file could be made available by user in the container, may need to be support it.
		if credSource.File != "" && credSource.File != autoMountedServiceAccountTokenPath {
			return fmt.Errorf("%s: credential_source.file \"%s\" is not a recognized path", invalidConfigErrPrefix, credSource.File)
		}
		if credSource.EnvironmentID == "" && credSource.URL != wif.ExternalTokenEndpoint {
			return fmt.Errorf("%s: credential_source.url \"%s\" does not match with the configured %s externalTokenEndpoint", invalidConfigErrPrefix, credSource.URL, wif.ExternalTokenEndpoint)
		}
		if credSource.EnvironmentID != "" {
			if !strings.HasPrefix(strings.ToLower(credSource.EnvironmentID), awsEnvironmentIDPrefix) {
				return fmt.Errorf("%s: credential_source.environment_id \"%s\" must start with %s", invalidConfigErrPrefix, credSource.EnvironmentID, awsEnvironmentIDPrefix)
			}
			if !awsSTSTokenURLRegex.MatchString(credSource.URL) {
				return fmt.Errorf("%s: credential_source.aws.url \"%s\" does not have expected value", invalidConfigErrPrefix, credSource.URL)
			}
			if !awsRegionURLRegex.MatchString(credSource.RegionURL) {
				return fmt.Errorf("%s: credential_source.aws.region_url \"%s\" does not have expected value", invalidConfigErrPrefix, credSource.RegionURL)
			}
			if !awsSessionTokenURLRegex.MatchString(credSource.IMDSv2SessionTokenURL) {
				return fmt.Errorf("%s: credential_source.aws.imdsv2_session_token_url \"%s\" does not have expected value", invalidConfigErrPrefix, credSource.IMDSv2SessionTokenURL)
			}
		}
	}
	return nil
