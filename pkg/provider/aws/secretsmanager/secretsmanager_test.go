...
func TestPushAndRetrieveSecret(t *testing.T) {
	// Setup
	...
	arn := "arn:aws:secretsmanager:us-east-1:123456789012:secret:test-remote-key"
	secretOutput := &awssm.CreateSecretOutput{ARN: &arn}
	putSecretOutput := &awssm.PutSecretValueOutput{ARN: &arn}
	// getSecretOutput := &awssm.GetSecretValueOutput{SecretString: aws.String(string(secretValue))}

	fakeClient := fakesm.Client{
		GetSecretValueWithContextFn: fakesm.NewGetSecretValueWithContextFn(&awssm.GetSecretValueOutput{}, nil),
		CreateSecretWithContextFn:   fakesm.NewCreateSecretWithContextFn(secretOutput, nil),
		PutSecretValueWithContextFn: fakesm.NewPutSecretValueWithContextFn(putSecretOutput, nil),
		// DescribeSecretWithContextFn: fakesm.NewDescribeSecretWithContextFn(tagSecretOutput, nil),
	...
	pushSecretData := fake.PushSecretData{SecretKey: secretKey, RemoteKey: "test-remote-key"}
	...
	// fakeClient.CreateSecretWithContextFn = fakesm.NewCreateSecretWithContextFn(secretOutput, nil)
	// fakeClient.PutSecretValueWithContextFn = fakesm.NewPutSecretValueWithContextFn(putSecretOutput, nil)
	// fakeClient.GetSecretValueWithContextFn = fakesm.NewGetSecretValueWithContextFn(getSecretOutput, nil)
	...
	// Retrieve the secret
	remoteRef := esv1beta1.ExternalSecretDataRemoteRef{Key: "test-remote-key"}
	retrievedSecret, err := sm.GetSecret(context.Background(), remoteRef)
	assert.NoError(t, err)
	assert.Equal(t, secretValue, retrievedSecret)

}

func TestCreateSecretWithMetadata(t *testing.T) {
	secretKey := "fake-secret-key"
	secretValue := []byte("fake-value")
	arn := "arn:aws:secretsmanager:us-east-1:702902267788:secret:foo-bar5-Robbgh"
	fakeSecret := &corev1.Secret{
	...
	err := sm.PushSecret(context.Background(), fakeSecret, pushSecretDataWithMetadata)
	assert.NoError(t, err)

	ere, _ := sm.GetAllSecrets(context.Background(), esv1beta1.ExternalSecretFind{
		// Name: &esv1beta1.FindName{
		// 	RegExp: "fake-secret-key",
		// },
		// Name: "foo-bar5-Robbgh",
		// Tags: map[string]string{
		// 	"tagname12": "tagvalue1",
		// },
	})
	fmt.Println("GetAllSecrets", ere)
	fmt.Println("GetRemoteKey:", pushSecretDataWithMetadata.GetRemoteKey())

	remoteRef := esv1beta1.ExternalSecretDataRemoteRef{Key: pushSecretDataWithMetadata.GetRemoteKey()}
	retrievedSecret, err := sm.GetSecret(context.Background(), remoteRef)
	fmt.Println("retrievedSecret", retrievedSecret)

	// ta, e := sm.GetSecret(context.Background(), esv1beta1.ExternalSecretDataRemoteRef{
	// 	Key:     pushSecretDataWithMetadata.RemoteKey,
	// 	Version: "AWSCURRENT",
	// })
	// assert.NoError(t, e)
	// fmt.Println("========")
...
func TestCreateSecretWithoutMetadataAndUpdateWithMetadata(t *testing.T) {
	t.Skip()
}
