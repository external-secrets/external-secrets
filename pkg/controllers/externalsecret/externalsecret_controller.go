...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (result ctrl.Result, err error) {
	...
	// patch status of the ExternalSecret when this function returns, if needed.
	...
	prevExternalSecret := *externalSecret.DeepCopy()

	defer func() {
		// if the status has not changed, we don't need to update it
		if equality.Semantic.DeepEqual(prevExternalSecret.Status, externalSecret.Status) {
			return
		}

		// patch the status of the ExternalSecret, storing any error in a new variable
		// if there was no new error, we don't need to change the `result` or `err` values
		updateErr := r.Status().Patch(ctx, externalSecret, client.MergeFrom(&prevExternalSecret))
		if updateErr == nil {
			return
		}
