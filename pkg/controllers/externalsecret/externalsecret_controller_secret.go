...
func (r *Reconciler) getProviderSecretData(ctx context.Context, externalSecret *esv1beta1.ExternalSecret) (map[string][]byte, error) {
	...
	genState := statemanager.New(externalSecret)
	defer func() {
		if err != nil {
	...
	for i, remoteRef := range externalSecret.Spec.DataFrom {
		...
		if remoteRef.Find != nil {
			...
			secretMap, err = r.handleGenerateSecrets(ctx, externalSecret, remoteRef, i, genState)
		}
...
func (r *Reconciler) handleGenerateSecrets(ctx context.Context, es *esv1beta1.ExternalSecret, remoteRef esv1beta1.ExternalSecretDataFromRemoteRef, i int, generatorState *statemanager.Manager) (map[string][]byte, error) {
	gen, genResource, err := resolvers.GeneratorRef(ctx, r.RestConfig, es.Namespace, remoteRef.SourceRef.GeneratorRef)
	if err != nil {
		return nil, fmt.Errorf("unable to resolve generator: %w", err)
	}
	latestState := generatorState.GetLatest(generatorStateKey(i))
	secretMap, newState, err := gen.Generate(ctx, genResource, r.Client, es.Namespace)
	if err != nil {
		return nil, fmt.Errorf(errGenerate, i, err)
	}
	if latestState != nil {
		generatorState.MoveStateToGC(genResource, generatorStateKey(i), gen, latestState)
	}
	generatorState.SetLatest(ctx, r.Client, generatorStateKey(i), es.Namespace, genResource, gen, newState)
...
func (r *Reconciler) handleExtractSecrets(ctx context.Context, externalSecret *esv1beta1.ExternalSecret, remoteRef esv1beta1.ExternalSecretDataFromRemoteRef, cmgr *secretstore.Manager, genState *statemanager.Manager, i int) (map[string][]byte, error) {
	...
	genState.FlagLatestStateForGC(generatorStateKey(i))
