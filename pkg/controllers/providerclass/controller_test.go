...
func TestProviderClassReconcileMarksReadyWhenHealthCheckSucceeds(t *testing.T) {
	scheme := runtime.NewScheme()
	utilruntime.Must(clientgoscheme.AddToScheme(scheme))
	utilruntime.Must(esv1alpha1.AddToScheme(scheme))

	obj := &esv1alpha1.ProviderClass{
		ObjectMeta: metav1.ObjectMeta{Name: "aws", Namespace: "team-a"},
	...
	_, err := r.Reconcile(context.Background(), ctrl.Request{
		NamespacedName: types.NamespacedName{Name: "aws", Namespace: "team-a"},
	})
	if err != nil {
		t.Fatalf("Reconcile() error = %v", err)
	}

	updated := &esv1alpha1.ProviderClass{}
	if err := kubeClient.Get(context.Background(), types.NamespacedName{Name: "aws", Namespace: "team-a"}, updated); err != nil {
